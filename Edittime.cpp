...
PROPS_DATA_START()
    ...
    // PropData_Button(PROPID_EXCLUDE, PSTR(""), PSTR("Set the area where objects will be excluded from the level file when saving."), PSTR("Setup Object Exclusion Zone")),
...
BOOL WINAPI DLLExport EditProp(LPMV mV, LPEDATA edPtr, UINT nPropID)
{
    ...
    //if (nPropID==PROPID_SAVE)
    //{
    //    TCHAR szFilters[] = _T("Stone Goose Level Files (*.sglev)\0*.sglev\0\0");
    //    TCHAR szFilePathName[_MAX_PATH] = _T("");
    //    OPENFILENAME ofn = {0};
    //    ofn.lStructSize = sizeof(OPENFILENAME);
    //    ofn.hwndOwner = mV->mvHMainWin;
    ...
    //       
    //    }
    //    CRunApp* test = ((CRunApp*)mV->mvEditApp);
    //    return TRUE;
    //}
	// Example
	// -------
...
//	int error = -1;
//#ifndef RUN_ONLY
//    pIconSf->Fill(255,255,255);
//
//	//if ( pIconSf->LoadImage(hInstLib, EXO_ICON) != 0 )
//	error = 0;
//
//    LPCSTR text = "Exclusion Zone"; // text to print
//    DWORD dwCharCount = -1; // number of characters to print. If dwCharCount is -1, the text must be terminated with a 0.
//    int x = edPtr->swidth; // X coordinate to position the text string
//    int y = edPtr->sheight; // Y coordinate to position the text string
//    DWORD alignMode = TEXT_ALIGN_BOTTOM|TEXT_ALIGN_RIGHT; // alignment flags described in the Windows SetTextAlign function
//        
//    tagRECT t;
//    t.left = 0;
//    t.right = edPtr->swidth;
//    t.top = 0;
//    t.bottom = edPtr->sheight;
//        
//    LPRECT pClipRc = (LPRECT)&t; // clipping rectangle
//    COLORREF color = 0xFFFFFF; // color
//    HFONT hFnt = NULL; // text font
//    BlitMode bm = BMODE_TRANSP; // ignored, the text is always displayed in transparent mode
//    BlitOp bo = BOP_COPY; // ink effect
//    LPARAM param = 0; // ink effect parameter
//    int AntiA = 0; // anti-aliasing if TRUE
//
//
//    pIconSf->TextOutA(text,dwCharCount, x, y, alignMode, pClipRc, color, hFnt, bm, bo, param=0, AntiA=0);
//
//
//#endif // !RUN_ONLY
//	return error;
...
int WINAPI DLLExport CreateObject(mv _far *mV, fpLevObj loPtr, LPEDATA edPtr)
{
    ...
    LPOI list = (LPOI)loPtr->loOiHandle;
    edPtr->swidth = 128;
    edPtr->sheight = 128;
...
{
#ifndef RUN_ONLY
	edPtr->swidth = max(128,cx);
	edPtr->sheight = max(128,cy);
#endif // !RUN_ONLY
	return TRUE;	// OK
}
